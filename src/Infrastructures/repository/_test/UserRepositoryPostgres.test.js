...
describe('UserRepositoryPostgres', () => {
  ...
  describe('getUserById', () => {
    it('should throw InvariantError when user id not found', async () => {
      ...
      await expect(userRepositoryPostgres.getUserById('user-123')).rejects.toThrowError(InvariantError);
    });

    it('should return user data correctly', async () => {
      await UsersTableTestHelper.addUser({ id: 'user-123', username: 'johndoee' });
      const userRepositoryPostgres = new UserRepositoryPostgres(pool, {});

      const user = await userRepositoryPostgres.getUserById('user-123');

      expect(user.id).toEqual('user-123');
      expect(user.username).toEqual('johndoee');
    });
  });
});
